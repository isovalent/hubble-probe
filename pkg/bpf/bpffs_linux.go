...
import (
	...
	"github.com/covalentio/hubble-probe/pkg/mountinfo"
)
...
func checkOrMountDefaultLocations() error {
	...
	if !bpffsInstance {
		...
		if !cMounted {
			...
			fmt.Printf("%s is mounted but has a different filesystem than BPFFS", "/run/cilium/bpffs")
		}
	}
